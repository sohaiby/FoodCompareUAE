...
exports.deliverooAdapter = exports.DeliverooAdapter = void 0;
const db_1 = require("../db");
// Predefined Deliveroo city & neighborhood mapping for UAE seed zones
const DELIVEROO_AREAS = {
...
class DeliverooAdapter {
    ...
    inferCuisines(name) {
        ...
        if (lower.includes('burger') || lower.includes('hardee') || lower.includes('mcdonald'))
            return ['Burgers', 'American', 'Fast Food'];
        if (lower.includes('shawarma') || lower.includes('laffah') || lower.includes('falafel') || lower.includes('moqren'))
            return ['Arabic', 'Shawarma', 'Middle Eastern'];
        if (lower.includes('mandi') || lower.includes('maraheb') || lower.includes('yemen'))
            return ['Mandi', 'Yemeni', 'Khaleeji'];
        if (lower.includes('chicken') || lower.includes('kfc') || lower.includes('chicking'))
            return ['Fried Chicken', 'Fast Food'];
        if (lower.includes('chinese') || lower.includes('chin') || lower.includes('panda'))
            return ['Chinese', 'Asian', 'Noodles'];
        if (lower.includes('coffee') || lower.includes('starbucks') || lower.includes('cafe'))
            return ['Coffee', 'Beverages', 'Bakery'];
        if (lower.includes('ice cream') || lower.includes('baskin') || lower.includes('stone'))
            return ['Desserts', 'Ice Cream'];
        if (lower.includes('salad') || lower.includes('healthy') || lower.includes('500 calorie'))
            return ['Healthy', 'Salads', 'Bowls'];
        if (lower.includes('indian') || lower.includes('biryani') || lower.includes('gazebo'))
            return ['Indian', 'Biryani', 'Curry'];
...
exports.DeliverooAdapter = DeliverooAdapter;
exports.deliverooAdapter = new DeliverooAdapter();
