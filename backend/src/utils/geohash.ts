import { LocationCell, SEED_LOCATIONS, DEFAULT_LOCATION } from '@uae-food-compare/shared';
 ...
 */
export function resolveLocationCell(lat?: number, lng?: number, requestedGeohash?: string): LocationCell {
  if (requestedGeohash) {
    const matched = SEED_LOCATIONS.find((loc) => loc.geohash.startsWith(requestedGeohash) || requestedGeohash.startsWith(loc.geohash));
