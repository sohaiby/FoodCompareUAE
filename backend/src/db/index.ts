...
export interface DBRecord {
  ...
  menuItems: MenuItem[];
  platformFees: Record<string, Record<PlatformId, DeliveryFee>>; // restaurantId -> platform -> fee
  platformItemPrices: Record<string, Record<PlatformId, { basePriceAed: number; discountPercent?: number; isAvailable: boolean }>>; // itemId -> platform -> price
  offers: Record<string, OfferBadge[]>; // restaurantId -> offers
  platformGlobalOffers: Record<PlatformId, OfferBadge[]>; // platform -> platform-wide offers
  gridCells: Record<string, LocationCell>; // geohash -> LocationCell
  gridCellRestaurants: Record<string, string[]>; // geohash -> restaurantIds
}
...
class DatabaseService {
  ...
  public getRestaurantsByGeohash(geohash: string): Restaurant[] {
    const matchedCell = Object.keys(this.data.gridCellRestaurants).find(
      (key) => geohash.startsWith(key) || key.startsWith(geohash)
    );
  ...
  public getMenuItemsByRestaurant(restaurantId: string): MenuItem[] {
  ...
  public getPlatformDeliveryFee(restaurantId: string, platform: PlatformId): DeliveryFee {
  ...
  public getPlatformItemPricing(menuItemId: string, platform: PlatformId) {
  ...
  public getOffersForRestaurant(restaurantId: string, platform: PlatformId): OfferBadge[] {
  ...
  public getGridCell(geohash: string): LocationCell | undefined {
    return this.data.gridCells[geohash];
  }

  public updateGridCell(cell: LocationCell, restaurantIds?: string[]) {
   ...
   */
  public upsertRestaurants(geohash: string, incomingRestaurants: Restaurant[]): void {
    ...
    for (const inc of incomingRestaurants) {
      ...
      let existing = this.data.restaurants.find(
        (r) => r.id === inc.id || r.name.trim().toLowerCase() === normName
      );
   ...
   */
  private ensureMenuForRestaurant(restaurant: Restaurant): void {
    ...
    const platforms: PlatformId[] = ['talabat', 'noon', 'deliveroo', 'careem', 'keeta', 'smiles'];
    if (!this.data.platformFees[restaurant.id]) {
      this.data.platformFees[restaurant.id] = {} as any;
    }
  ...
  private generateItemsForCuisine(restaurant: Restaurant): MenuItem[] {
  ...
  public seedDatabase(data: DBRecord) {
    this.data = data;
    this.save();
  }
