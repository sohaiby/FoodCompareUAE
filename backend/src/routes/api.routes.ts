...
import { resolveLocationCell, encodeGeohash } from '../utils/geohash';
...
export async function apiRoutes(fastify: FastifyInstance, _opts: FastifyPluginOptions) {
  // Health & Stats
  ...
  fastify.get<{
    Querystring: { lat?: string; lng?: string; geohash?: string };
  }>('/locations/resolve', async (request) => {
  ...
  fastify.post<{
    Body: RefreshCellRequest;
  }>('/refresh-cell', async (request) => {
